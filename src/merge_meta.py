...
if __name__ == '__main__':
    ...
    parser.add_argument('--startDate', '-s', help='Start date')
    parser.add_argument('--endDate', '-e', help='End date', default=date.today())
    parser.add_argument('--threshold', '-t', help='threshold', default=0)
    parser.add_argument('--mutFile', '-u', help='mutation list (csv)', required=True)
    parser.add_argument('--prefix','-p', help='outputs perfix')
    parser.add_argument('--days', '-d', help='sliding window(day)', default=14)
    parser.add_argument('--cat', '-c', help='categorization', default='country')
    ...
    if args.days is None or args.days == '':
        ...
        try:
            minDays = int(args.days)
        except ValueError:
            minDays = 14

    # Ensure userColumn is set correctly
    if args.cat is None or args.cat == '':
        userColumn = "country"
    else:
        userColumn = args.cat
    ...
    start_datetime = datetime.strptime(str(startDate), "%Y-%m-%d")
    end_datetime = datetime.strptime(str(endDate), "%Y-%m-%d")
