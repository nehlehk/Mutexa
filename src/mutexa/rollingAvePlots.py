...
def run_plots(
    ...
    print(f'\033[1mExecuting: {run_plots.__name__}\033[0m')  # green
    ...
    for m in (pbar:= tqdm(mutations)):
        pbar.set_postfix_str(m)
        pbar.set_description_str('Calculating mutations rolling average')
    ...
    for m in mapp:
        if not (mapp[m] is None):
            ...
            plotly_dict = { # Save json to plot in notebook
                    'type': 'heatmap',
            ...
            if 'country' in results.columns:
                ...
                results = results.sort_values([userColumn2])
    ...
    #print("Rolling Pass",rolling_pass)
    
    counts = {}
    for m in tqdm(rolling_pass, desc='Calculating rolling averages'):
        ...
        countdict = dict.fromkeys(rolling_pass)
    ...
    print('\n','Saving heatmaps for: ',len(heatmapmatrix.keys()),heatmapmatrix.keys())
    ...
    for m in rolling_pass: # Carol - added to use only mutation sums >10 to avoid plotting rolling averages that are too small
        ...
        if not result.empty:
            ...
            plot.set_xlim(datetime.datetime.strptime(str(start), '%Y-%m-%d'),datetime.datetime.strptime(str(end), '%Y-%m-%d'))# result.index.max().date()) # Carol - previously changed to same start, end for each mutation
            plot.xaxis.set_major_locator(mdates.WeekdayLocator(interval=4)) # 4 Weeks between ticks
